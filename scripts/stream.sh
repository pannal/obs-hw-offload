...
start_ffmpeg() {
    ...
    docker run --tty --init --name "$CONTAINER_NAME" --device "$VAAPI_DEVICE:$VAAPI_DEVICE" "$DOCKER_IMAGE" \
        ffmpeg "${FFMPEG_PARAMS_ARRAY[@]}" &
    FFMPEG_PID=$!
    echo "FFmpeg is running with PID: $FFMPEG_PID"
}

# Function to monitor NDI source using ffprobe in Docker
monitor_source() {
    while true; do
        docker run --rm --name "$CHECK_CONTAINER_NAME" --device "$VAAPI_DEVICE:$VAAPI_DEVICE" "$DOCKER_IMAGE" \
...
# Function to kill the FFmpeg container
kill_ffmpeg() {
    echo "Stopping FFmpeg container..."
